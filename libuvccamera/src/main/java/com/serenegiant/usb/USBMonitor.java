...
//			if (DEBUG) Log.i(TAG, "unregister:");
	...
	private boolean updatePermission(final UsbDevice device, final boolean hasPermission) {
		// Fix: don't use serial number (useNewAPI=false) to avoid SecurityException on newer Androids
		// when we don't have permission yet.
		...
		if (useNewAPI && BuildCheck.isAndroid5()) {
			...
			if (TextUtils.isEmpty(serial)) {
				try {
					...
					// Fix: Ignore SecurityException if we don't have permission yet
					sb.append("#");
				}
			}
...
//		if (DEBUG) Log.v(TAG, "getDeviceKeyName:" + sb.toString());
	...
	/**
	 * 指定したID의 Stringディスクリプタから文字列を取得する。取得できなければnull
	 * @param connection
	 * @param id
	 * @param languageCount
	 * @param languages
