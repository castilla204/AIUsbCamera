...
#define FRAME_POOL_SZ MAX_FRAME + 20 // Increased frame pool size for varied sizes
...
//	
    ...
    // Preallocate frames for various expected sizes to reduce runtime allocations
    const size_t maxRGBXBytes = 2592 * 1944 * 4; // Max resolution * 4 bytes/pixel for RGBX (20.1MB)
    const size_t maxYUYVBytes = 2592 * 1944 * 2; // Max resolution * 2 bytes/pixel for YUYV (10.0MB)
    const size_t typicalMJPEGBytes = 5 * 1024 * 1024; // 5MB for compressed MJPEG
    const size_t defaultPreviewYUYVBytes = DEFAULT_PREVIEW_WIDTH * DEFAULT_PREVIEW_HEIGHT * 2; // 640*480*2 = 614KB

    // Allocate a few of each critical size
    add_frames_to_pool(maxRGBXBytes, 8); // More RGBX frames
    add_frames_to_pool(maxYUYVBytes, 8); // More YUYV frames
    add_frames_to_pool(typicalMJPEGBytes, 5); // Some large MJPEG frames
    add_frames_to_pool(defaultPreviewYUYVBytes, 3); // Some default preview frames
...
/**
 * Helper to add frames to the pool up to FRAME_POOL_SZ
 */
void UVCPreview::add_frames_to_pool(size_t data_bytes, int count) {
    pthread_mutex_lock(&pool_mutex);
    {
        for (int i = 0; i < count; i++) {
            if (mFramePool.size() < FRAME_POOL_SZ) {
                ...
                LOGW("Frame pool is full, cannot preallocate more frames.");
                break;
            }
        }
    }
 ...
 * if pool is empty or no suitable frame found, create new frame
 */
uvc_frame_t *UVCPreview::get_frame(size_t data_bytes) {
	uvc_frame_t *frame = NULL;
	pthread_mutex_lock(&pool_mutex);
	{
		// Search for a frame with sufficient size
		for (int i = 0; i < mFramePool.size(); ++i) {
			if (mFramePool[i] && mFramePool[i]->data_bytes >= data_bytes) { // Check for sufficient size
	...
	pthread_mutex_unlock(&pool_mutex);
	if UNLIKELY(!frame) {
		LOGD("allocate new frame");
		frame = uvc_allocate_frame(data_bytes);
	} else {
        // LOGD("reusing frame from pool"); // Uncomment for debug logging
    }
	return frame;
}

void UVCPreview::recycle_frame(uvc_frame_t *frame) {
	if (UNLIKELY(!frame)) return; // Sanity check

	pthread_mutex_lock(&pool_mutex);
	if (LIKELY(mFramePool.size() < FRAME_POOL_SZ)) {
		mFramePool.put(frame);
	} else {
		// If pool is full, free the frame to prevent memory leak
...
int UVCPreview::setPreviewSize(int width, int height, int min_fps, int max_fps, int mode, float bandwidth) {
	...
	if ((requestWidth != width) || (requestHeight != height) || (requestMode != mode)) {
		...
		// Pre-set frame geometry so that setPreviewDisplay() (which may be called
		// before prepare_preview() runs on the preview thread) configures the
		// ANativeWindow buffer with the requested size. Without this, defaults of
		// 640x480 leak into ANativeWindow_setBuffersGeometry and the SurfaceView
		// renders the larger frames with the wrong stride (visible as the image
		// being "cut in half"). prepare_preview will overwrite these with the
		// negotiated values from the camera, which for a sane request are equal.
		frameWidth = width;
		frameHeight = height;
		...
		if (!env->IsSameObject(mFrameCallbackObj, frame_callback_obj))	{
			...
			if (frame_callback_obj) {
				...
				if (LIKELY(clazz)) {
					...
					env->DeleteLocalRef(clazz); // MODIFIED: release local reference to prevent leak
				} else {
...
int UVCPreview::startPreview() {
	...
	if (!isRunning()) {
		...
		pthread_mutex_lock(&preview_mutex);
		{
            // MODIFIED: allow preview thread without display surface (headless mode)
		...
		if (frameMode) {
			...
			for ( ; LIKELY(isRunning()) ; ) {
				...
				if (LIKELY(frame_mjpeg)) {
					...
					if (LIKELY(!result)) {
						// MODIFIED: pass NULL window if it doesn't exist.
						frame = draw_preview_one(frame, mPreviewWindow ? &mPreviewWindow : NULL, uvc_any2rgbx, 4);
...
uvc_frame_t *UVCPreview::draw_preview_one(uvc_frame_t *frame, ANativeWindow **window, convFunc_t convert_func, int pixcelBytes) {
    ...
    // MODIFIED: This function is now responsible for conversion, drawing is optional.
    // It will return the ORIGINAL frame, not the converted one. The converted one is recycled internally.
	int b = 0;
    // MODIFIED: Check if window exists before trying to lock the preview_mutex.
	if (window && *window) {
		pthread_mutex_lock(&preview_mutex);
		{
			b = *window != NULL;
    ...
    if (convert_func) {
        ...
        if LIKELY(converted) {
            ...
            if (!b) {
                // If conversion was successful, and we have a window, draw on it.
                if (LIKELY(window && *window)) {
                    pthread_mutex_lock(&preview_mutex);
        ...
        // No conversion function, just draw the raw frame if a window exists.
        if (LIKELY(window && *window)) {
            pthread_mutex_lock(&preview_mutex);
...
void UVCPreview::do_capture(JNIEnv *env) {
	...
	clearCaptureFrame();
	// MODIFIED: removed call to callbackPixelFormatChanged
	for (; isRunning() ;) {
		mIsCapturing = true;
		if (mCaptureWindow) {
...
void UVCPreview::do_capture_callback(JNIEnv *env, uvc_frame_t *frame) {
	...
	if (LIKELY(frame)) {
		...
		jobject callback_obj = NULL;
		jmethodID callback_method = NULL;

		// Step 1: Quick check under mutex to see if callback exists
		pthread_mutex_lock(&capture_mutex);
		has_callback = (mFrameCallbackObj != NULL && iframecallback_fields.onFrame != NULL);
		if (has_callback) {
			// Create NEW global reference that won't be deleted while we use it
			callback_obj = env->NewGlobalRef(mFrameCallbackObj);
			callback_method = iframecallback_fields.onFrame;
		}
		pthread_mutex_unlock(&capture_mutex);

		// Step 2: Do frame conversion outside of mutex (can take time)
		if (LIKELY(has_callback) && mFrameCallbackFunc) {
			...
			if (LIKELY(callback_frame)) {
				...
				if (UNLIKELY(b)) {
					...
					if (callback_obj) env->DeleteGlobalRef(callback_obj);
					goto SKIP;
				}
			} else {
		...
		// Step 3: Make JNI call with valid reference (protected by NewGlobalRef)
		if (LIKELY(callback_obj) && LIKELY(callback_method)) {
			jobject buf = env->NewDirectByteBuffer(callback_frame->data, callbackPixelBytes);
			env->CallVoidMethod(callback_obj, callback_method, buf);
			env->ExceptionClear();
			env->DeleteLocalRef(buf);
			env->DeleteGlobalRef(callback_obj);  // Clean up our new global ref
		}
 SKIP:
