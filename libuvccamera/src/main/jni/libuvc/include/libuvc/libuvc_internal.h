 ...
 */
/* FIX 2026-05-17: subido de 10 → 32. Upstream libuvc usa 100, pero saki4510t
 * lo bajó a 10 hace años cuando los teléfonos Android tenían poca RAM. En el
 * S10e (8 GB) + cámaras IMX179@1280×720@30fps MJPEG, 10 buffers se llenan en
 * ~20 ms de ráfaga; cuando esto ocurre libusb descarta el siguiente packet
 * y libuvc entrega un MJPEG con FID flipped sin EOF previo → frame truncado
 * que produce el "frame gris" (libuvc#122). Con 32 buffers el isochronous
 * endpoint resiste ~60 ms de presión sin perder packets. NO subir más de 32:
 * cada buffer pesa LIBUVC_XFER_BUF_SIZE = 16 MB → 32 buffers = 512 MB de
 * memoria reservada virtualmente (Linux la mapea bajo demanda) y satura el
 * DMA del Exynos 9820 si excedemos. */
#define LIBUVC_NUM_TRANSFER_BUFS 32
